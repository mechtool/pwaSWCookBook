...
export class AppComponent implements OnInit{
	...
	new SideNavItemClass('Введение', 'level0', '#adadad','/introduction', '1'),
        new SideNavItemClass('Стратегии кэширования', 'level0', '#adadad','/caching-strategies/network-cache', '2', 'remove', [
	    new SideNavItemClass('Сеть или кэш', 'level1', '#21a01a','/caching-strategies/network-cache/intro', 'Нв'),
	    new SideNavItemClass('Только кэш', 'level1', '#21a01a','/caching-strategies/cache-only', 'Нв'),
	    new SideNavItemClass('Кэш и обновление', 'level1', '#21a01a','/caching-strategies/cache-update', 'Нв'),
	    new SideNavItemClass('Кэш, обновление и отображение', 'level1', '#e16800','/caching-strategies/cache-update-refresh', 'Ср'),
	    new SideNavItemClass('Откат при отсутствии в сети', 'level1', '#e16800','/caching-strategies/embedded-fallback', 'Ср'),
	]),
	new SideNavItemClass('Сетевые уведомления', 'level0', '#adadad','/web-push', '3', 'remove',[
	    new SideNavItemClass('Отправка и запрос данных', 'level1', '#21a01a','/web-push/push-retrieve-payload', 'Нв'),
	...
	new SideNavItemClass('Общее использование', 'level0', '#adadad','/general-usage', '4', 'remove', [
	    new SideNavItemClass('Немедленный контроль', 'level1', '#21a01a','/general-usage/immediate-claim', 'Нв'),
	    new SideNavItemClass('Передача сообщений', 'level1', '#21a01a','/general-usage/message-relay', 'Нв'),
	    new SideNavItemClass('Получение ресурсов', 'level1', '#e16800','/general-usage/fetching-resources', 'Ср'),
	...
	this.communicateService.communicateObservable.subscribe((data)=>{ //вызывается при загрузки компонента в основной outlet
	    if(data.type == 'intro'){
		...
		let buttons = this.buttonPanel.nativeElement.querySelectorAll('a');
		//отчистка панели кнопок от установленного класса активности
		buttons.forEach((el, inx)=>{
		    el.classList.remove('activeMenuButton');
		    if(inx == data.button && !this.menuButtons[data.button].disable){  //установка активности кнопки панели
			el.classList.add('activeMenuButton');
		    }
		}) ;
	    }
	    //запуск рендеоенга
